...
summarization_type = st.sidebar.selectbox("Select Summarization Type:", options=["Text Summarization", "YouTube Video Summarization", "Audio Summarization"], key="summarization_type")

if summarization_type == "Text Summarization":
    st.header("Text Summarization")
    text_input = st.text_area("Enter Text:", key="text_input")
    if st.button("Summarize Text", key="summarize_text"):
        if text_input:
            summary = generate_gemini_content(text_input, prompt)
            if summary:
                st.markdown("## Summary")
                st.write(summary)

                # Download summary
                st.download_button(label="Download Summary", data=summary, file_name="summary.txt", mime="text/plain", key="download_summary_text")
...
elif summarization_type == "YouTube Video Summarization":
    ...
    if st.button("Get Detailed Notes", key="get_detailed_notes"):
        transcript_text = extract_transcript_details(youtube_link, transcript_language)
